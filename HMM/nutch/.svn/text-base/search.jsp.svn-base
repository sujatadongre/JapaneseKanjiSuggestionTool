...
<%
  ...
  //HMM_Parse_Jap v = new HMM_Parse_Jap();
...
//remove these comments for viterbi to run as before  
...
//ending those comments


Forward_Alpha fa = new Forward_Alpha();
String myStr = fa.callExp(request.getParameter("query"));
System.out.println("Did you mean Forward: " + myStr);


  // get query from request
  String queryString = request.getParameter("query");
  //String queryString = myStr;
  if (queryString == null)
    queryString = "";
  String htmlQueryString = Entities.encode(request.getParameter("query"));
...
<jsp:include page="<%= language + \"/include/header.html\"%>"/>
 ...
 <% } %>
 <%
 	if(request.getParameter("toshow") == null){
 %>
 	<p>Did you mean:<a href="../search.jsp?query=<%=myStr%>&toshow=0"><%=myStr%></a></p>
 <%
 	}
 %>
