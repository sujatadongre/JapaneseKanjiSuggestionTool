...
public class Viterbi_New {
	...
	public String callExp(String qry){	
		...
		//String userStr = "あなむ";
		 ...
		 String resultStr = ViterbiAlgo((String[]) probDetails.get(0), (double[])probDetails.get(1), (double[][])probDetails.get(2), (double[][])probDetails.get(3), qry, suggestedString);
		 System.out.println("resultStr => " + resultStr);
		 return resultStr;
	}
	
	public static String ViterbiAlgo(String[] states, double[] start_prob, 
...
//		}
		...
		if(arrJapChars!= null && arrJapChars.length > 0){
			...
			for(int a = 0; a < arrJapChars.length; a++){
				...
				for(int output = 0; output < obs.size(); output++){
					...
					for(int j = 0; j < states.length; j++){
						...
						for(int k = 0; k < states.length; k++){
							System.out.println("states[k] => " + states[k] + " T.get(states[k] => " + T.get(states[k]).toString());
							Object[] objs = T.get(states[k]);
							prob = (Double) objs[0];//start prob initially
							System.out.println("prob => " + prob);
							v_path = objs[1].toString();
							System.out.println("v_path => " + v_path);
							v_prob = (Double) objs[2];//start prob initially
				...
				//ProbComparator probComp = new ProbComparator();
				//probComp.setCh(currentObs);
				//probComp.setProb(valmax);
				//arrSortArr.add(probComp);
...
//			ArrayList<Double> col = (ArrayList<Double>) htJapChars.values();
//			Collections.sort(col, (Comparator<Double>)probComp);
			...
			for(int l = myArray.length-1; l >= 0; l--){
				...
				if(isCharFound == 1){
					System.out.println("**********1. Viterbi もうしかして? :" + suggestedString);
					break;
				}
			}
...
public static void main(String args[]){
	...
	String userStr = input.nextLine();
	System.out.println(userStr);
	String myStr = vNew.callExp(userStr);
