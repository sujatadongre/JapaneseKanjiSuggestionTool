...
public class Viterbi_Last_Append {
	
	public final static String suggestedString = null;
	public Viterbi_Last_Append(){
		//callExp();
	}
...
//		}
		...
		for(int s = 0; s < userStr.length(); s++){
...
//					}
					...
					for(int output = 0; output < obs.size(); output++){
						...
						//System.out.println("currentObs => " + currentObs + " obs.get(output) => " + obs.get(output));
